...
def generate_poem():
    ...
    try:
        logger.info("جاري الاتصال بـ DeepSeek API...")
        ...
        logger.info(f"تم توليد قصيدة بعنوان: {title}")
        ...
        logger.error("انتهت مهلة الاتصال بـ DeepSeek API")
        return get_fallback_poem()
    except requests.exceptions.RequestException as e:
        logger.error(f"خطأ في الاتصال بـ DeepSeek API: {e}")
        return get_fallback_poem()
    except Exception as e:
...
def get_fallback_poem():
    ...
    poem = random.choice(default_poems)
    logger.info(f"استخدام قصيدة افتراضية: {poem['title']}")
    return poem
...
def test_api_connection():
    ...
    try:
        ...
        response = requests.get(DEEPSEEK_API_URL, timeout=10)
...
if __name__ == "__main__":
    logger.info("=" * 50)
    logger.info("🚀 بدء تشغيل بوت نشر الشعر الساخر")
    logger.info("=" * 50)
    ...
    logger.info(f"🔗 API المستخدم: {DEEPSEEK_API_URL}")
